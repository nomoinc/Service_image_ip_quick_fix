...
class URLMigrationService:
    ...
    def __init__(self):
        ...
        self.groundtruth_collection = os.getenv(
            'GROUNDTRUTH_COLLECTION', 'imageUrl')
        self.user_clothes_collection = os.getenv(
    ...
    def process_groundtruth_collection(self):
        """Process documents in wearapp_groundtruth.imageUrl collection"""
        try:
            groundtruth_db = self.client['wearapp_groundtruth']
            collection = groundtruth_db[self.groundtruth_collection]
            ...
            query = {
                '$or': [
                    {field: {'$regex': self.old_url.replace(
                        '/', '\\/'), '$options': 'i'}}
            ...
            logger.info(
                f"Groundtruth: Found {total_matches} documents matching old URL")
            ...
            for doc in documents:
                doc_id = doc['_id']
                modified_doc, was_modified = self.replace_url_in_dict(
                    doc, url_fields)
            ...
            logger.info(
                f"Groundtruth: Updated {updated_count} documents out of {total_matches}")
            if updated_count > 0:
    ...
    def process_user_clothes_collection(self):
        """Process documents in wearapp_user.userUploadedClothes collection"""
        try:
            user_db = self.client['wearapp_user']
            collection = user_db[self.user_clothes_collection]
            ...
            total_matches = collection.count_documents(query)
            logger.info(
                f"User Clothes: Found {total_matches} documents matching old URL")
            ...
            for doc in documents:
                ...
                if was_modified:
                    ...
                    modified_doc['updatedAt'] = datetime.now(UTC)
            ...
            logger.info(
                f"User Clothes: Updated {updated_count} documents out of {total_matches}")
            if updated_count > 0:
    ...
    def run_check(self):
        ...
        from datetime import UTC
        self.stats['last_check'] = datetime.now(UTC)

        if groundtruth_count > 0 or user_clothes_count > 0:
            logger.info(
                f"Check complete - GT: {groundtruth_count}, UC: {user_clothes_count}")
    ...
    def print_stats(self):
        ...
        logger.info(
            f"  Groundtruth documents updated: {self.stats['groundtruth_updated']}")
        logger.info(
            f"  User clothes documents updated: {self.stats['user_clothes_updated']}")
